...
###############
  ...
  VesselConfig = c("Vessel"=1, "VesselYear"=0)
  ...
  # Determine region
  Region = "Celtic_Sea"
  Restrict_NWA_to_Albatross_Polyvalent <- FALSE
  Catch_units <- 'Kg'
...
################
  ...
  # Plot covariances
  Cov_List = Summarize_Covariance( report=Report, parhat=Save$ParHat, tmbdata=TmbData, sd_report=Opt$SD, plot_cor=FALSE, names_set=levels(DF[,'SpeciesName']), figname=paste0(DateFile,"Spatio-temporal_covariances"), plotTF=c("Omega1"=TRUE,"Epsilon1"=TRUE,"Omega2"=TRUE,"Epsilon2"=TRUE), mgp=c(2,0.5,0), tck=-0.02, oma=c(0,5,2,2) )
  ...
  # Plot index
  SpatialDeltaGLMM::PlotIndex_Fn( DirName=DateFile, TmbData=TmbData, Sdreport=Opt$SD, Year_Set=sort(unique(Data_Geostat[,'Year'])), strata_names=strata.limits[,1], category_names=levels(DF[,'spp']), use_biascorr=TRUE )
