...
# Trim years
  ...
  # DF <- DF[DF$Survey %in% c('CEXP','THA2'),]
...
#   Vess_Cov <- as.matrix(reshape2::dcast(as.data.frame(Data_Geostat), spp + Year + Lat + Lon + Catch_KG + AreaSwept_km2~ Vessel, value.var = 'Vessel', length)[as.character(c(1:7))])
...
################
  ...
  # Make TMB data list
  TmbData = Data_Fn("Version"=Version, "FieldConfig"=FieldConfig, "RhoConfig"=RhoConfig, "ObsModel"=ObsModel, "OverdispersionConfig" = OverdispersionConfig, "c_i"=as.numeric(Data_Geostat[,'spp'])-1, "b_i"=Data_Geostat[,'Catch_KG'], "a_i"=Data_Geostat[,'AreaSwept_km2'], "s_i"=Data_Geostat[,'knot_i']-1, "t_i"=Data_Geostat[,'Year'], "a_xl"=Spatial_List$a_xl, Q_ik = Vess_Cov, "MeshList"=Spatial_List$MeshList, "GridList"=Spatial_List$GridList, "Method"=Spatial_List$Method )
